...
#[cfg(feature = "std")]
impl<T> SerJson for std::collections::VecDeque<T>
where
    T: SerJson,
{
    fn ser_json(&self, s: &mut SerJsonState) {
...
#[cfg(feature = "std")]
impl<T> DeJson for std::collections::VecDeque<T>
where
    T: DeJson,
{
    fn de_json(
        s: &mut DeJsonState,
        i: &mut Chars,
    ) -> Result<std::collections::VecDeque<T>, DeJsonErr> {
        let mut out = std::collections::VecDeque::<T>::new();
        s.block_open(i)?;
        while s.tok != DeJsonTok::BlockClose {
...
impl DeJson for Duration {
    fn de_json(s: &mut DeJsonState, i: &mut Chars) -> Result<Duration, DeJsonErr> {
        let mut sec = None;
        let mut nanos = None;
        s.curly_open(i)?;
        while s.tok != DeJsonTok::CurlyClose {
            let k: String = DeJson::de_json(s, i)?;
            match k.as_str() {
                "secs" => {
                    if sec.is_some() {
